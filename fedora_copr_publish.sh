...
version_in_changelog=$(grep -E "system76-acpi-dkms \([[:digit:]]+\.[[:digit:]]+\.[[:digit:]]+" debian/changelog | head -1)
...
sed -i "s#^Version:    .*#Version:    $version#" system76-acpi-dkms.spec.rpkg
sed -i "s#^Release:    .*#Release:    $release#" system76-acpi-dkms.spec.rpkg
git commit -m"bump Version to: $version-$release" system76-acpi-dkms.spec.rpkg
...
srpm="$(ls .rpkg-build/system76-acpi-dkms-*.src.rpm)"
